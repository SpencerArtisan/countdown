...
spec = do
   describe "countdown results" $ do
      ...
      it "should find multiple results from two values" $ do
          countdown [1,2] `shouldBe` [Solution 1 "1", Solution 2 "2", Solution 3 "1+2", Solution (-1) "1-2", Solution 2 "1*2", Solution 1 "1", Solution 3 "2+1", Solution 1 "2-1", Solution 2 "2*1", Solution 2 "2/1"] 
